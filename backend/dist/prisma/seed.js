    ...
    for (let i = 0; i < 20; i++) {
        ...
        const user = await prisma.user.create({
            data: {
                ...
                password: '$argon2id$v=19$m=65536,t=3,p=4$ZXhhbXBsZXNhbHQ$J2mY+0eO4L5+Hq5I7J3vI1YqL5J8vI1YqL5J8vI1YqL',
                ...
                wallet: {
                    create: {
                        balanceInKobo: Math.floor(Math.random() * 10000000),
    ...
    for (let i = 0; i < Math.min(50, allListings.length); i++) {
        ...
        const listing = await prisma.listing.create({
            data: {
                ...
                isSwapOnly: listingData.isSwapOnly || false,
                acceptsCash: listingData.acceptsCash !== false,
                acceptsSwap: true,
    ...
    for (let i = 0; i < 20; i++) {
        ...
        if (sender.id !== receiver.id) {
            await prisma.transaction.create({
                data: {
                    type: ['PURCHASE', 'SALE', 'ESCROW_DEPOSIT'][Math.floor(Math.random() * 3)],
                    amountInKobo: Math.floor(Math.random() * 50000000) + 1000000,
                    status: ['COMPLETED', 'PENDING', 'FAILED'][Math.floor(Math.random() * 3)],
                    paymentMethod: ['BANK_TRANSFER', 'CARD', 'WALLET'][Math.floor(Math.random() * 3)],
                    paymentProvider: ['PAYSTACK', 'FLUTTERWAVE'][Math.floor(Math.random() * 2)],
